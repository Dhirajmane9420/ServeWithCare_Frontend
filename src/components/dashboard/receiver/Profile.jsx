...
const ProfileSection = () => {
  ...
  const [user, setUser] = useState({ name: '', email: '', contact: '', address: '', ngo: '' });
  ...
  return (
    <div id="profile" className="bg-white dark:bg-dark-bg-secondary p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold text-primary-dark dark:text-white mb-6">Receiver Profile</h2>
      
      {isEditing ? (
        /* --- EDITING VIEW (FORM) --- */
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-dark dark:text-text-light">Organization Name</label>
            <input
              type="text" name="name"
              value={formData.name}
          ...
          <div>
            <label className="block text-sm font-medium text-text-dark dark:text-text-light">Affiliated NGO (Optional)</label>
            <input
              type="text" name="ngo"
              value={formData.ngo}
              onChange={handleInputChange}
        ...
        /* --- DISPLAY VIEW (STATIC) --- */
        <div className="space-y-4">
          <p className="text-text-dark dark:text-text-light"><strong>Organization Name:</strong> {user.name}</p>
          <p className="text-text-dark dark:text-text-light"><strong>Affiliated NGO:</strong> {user.ngo || 'Not set'}</p>
